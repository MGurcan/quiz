      ...
      <Fade>
        {leagueLogos.map((fadeImage, index) => (
          <div key={index} className='w-full h-full flex flex-col justify-center items-center'>
            ...
            <Link className="rounded-md w-4/5 bg-gradient-to-r from-yesil to-mavi text-gray-light text-center text-[32px] mt-4 hover:opacity-70 max-md:text-[20px]" to={`thisOrThat/${fadeImage.link}`}>Play</Link>
          </div>
        ))}
      </Fade>
    </div>
